...
async function carregarSecretarias() {
  try {
    ...
    mostrarErroBanner(
      "Não foi possível carregar as secretarias. Recarregue a página.",
    );
  }
}
...
function validarCampo(id, errId, condicao, mensagem) {
  ...
  if (!condicao) {
    ...
    if (erro) {
      ...
      erro.classList.remove("is-hidden");
    }
    return false;
  }
...
["descricao", "tipo", "estadoConservacao", "secretariaId"].forEach((id) => {
  document
    .getElementById(id)
    ?.addEventListener("input", () => limparErrosCampo(id, `err-${id}`));
  document
    .getElementById(id)
    ?.addEventListener("change", () => limparErrosCampo(id, `err-${id}`));
});
...
function validarFormulario() {
  ...
  const v4 = validarCampo(
    ...
    "err-secretariaId",
    !!secId,
    "Selecione a secretaria detentora.",
  );
...
function montarPayload() {
  ...
  Object.entries(opcional).forEach(([key, val]) => {
...
btnSalvar.addEventListener("click", async () => {
  ...
  try {
    ...
    window.location.replace(
      `patrimonio-detalhe.html?id=${patrimonio.id}&novo=1`,
    );
  } catch (err) {
    if (err.isValidation && err.errors) {
      ...
      mostrarErroBanner(
        err.message || "Erro ao salvar o bem. Tente novamente.",
      );
    }
    window.scrollTo({ top: 0, behavior: "smooth" });
  } finally {
